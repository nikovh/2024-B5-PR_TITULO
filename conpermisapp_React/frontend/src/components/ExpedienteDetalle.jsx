...
//     const handleSavePropietario = async (propietarioData, token) => {
...
const ExpedienteDetalle = () => {
    const { id } = useParams();
    const { datos, error } = useFetchDatos(id); // Hook para obtener datos iniciales
    const [isEditing, setIsEditing] = useState(false);
    const navigate = useNavigate();
    ...
    const handleSavePropietario = async (propietarioData, token) => {
        try {
            ...
            if (response.ok) {
                ...
                console.error("Error en el servidor al actualizar propietario.");
                alert("No se pudo actualizar el propietario.");
            }
        } catch (err) {
            console.error("Error al guardar el propietario:", err);
            alert("Error al guardar los datos del propietario.");
        }
    };

    const handleSavePropiedad = async (propiedadData, token) => {
        const url = datos.propiedad
            ? `http://localhost:4000/propiedades/${datos.propiedad.rolSII}`
            : `http://localhost:4000/propiedades`;

        try {
            const response = await fetch(url, {
                method: datos.propiedad ? "PUT" : "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
                },
                body: JSON.stringify({ ...propiedadData, expedienteId: datos.expediente.expedienteId }),
            });
    ...
    return (
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "20px" }}>
            ...
            <Desplegable title="Datos del Arquitecto Patrocinante">
                <ArquitectoPatrocinante usuario={datos.usuario} />
            </Desplegable>
